...
import { useGetAttendance } from "@/src/queries/attendance.query";
import { ActivityIndicator, FlatList, ListRenderItem, Text, TouchableOpacity, View } from "react-native";

export default function () {
    const { data, isLoading, refetch } = useGetAttendance();

    const renderAttendance: ListRenderItem<IAttendance> = ({ item }) => (
      <TouchableOpacity style={{ gap: 10 }} className="w-full flex-row items-center py-3 justify-start">
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text className="font-bold text-gray-700">{new Date(item.created_at).toDateString()}</Text>
          <Text className="text-gray-500">{item.course.name}</Text>

          <View style={{ gap: 5 }} className="flex-row mt-4 items-center justify-start">
            <Text className="text-gray-500">{item.timetable.weekday}</Text>
            <Text className={`${item.is_present ? 'text-green-500' : 'text-red-500'}`}>
